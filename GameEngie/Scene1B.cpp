...
//-----------------------------------------------------------------------------
// NAME: Scene1B::Scene1B(LPDIRECT3DDEVICE9 D3dDevice)
// DESC: ステージシーン処理 コンストラクタ
//-----------------------------------------------------------------------------
Scene1B::Scene1B(LPDIRECT3DDEVICE9 D3dDevice)
{
	pDevice = D3dDevice;
...
//-----------------------------------------------------------------------------
// NAME: HRESULT Scene1B::InitScene()
// DESC: ステージシーン処理 ステージシーン読込み
//-----------------------------------------------------------------------------
HRESULT	Scene1B::InitScene()
{
	// {カメラの初期化}
...
//-----------------------------------------------------------------------------
// NAME: VOID Scene1B::UpdateController()
// DESC: ステージシーン処理 ステージシーンコントローラーの総更新処理
//-----------------------------------------------------------------------------
VOID Scene1B::UpdateController()
{
	// {ゲームコントローラーカメラ修正更新}
...
//-----------------------------------------------------------------------------
// NAME: VOID Scene1B::UpdateStage()
// DESC: ステージシーン処理 ステージシーンの総更新処理
//-----------------------------------------------------------------------------
VOID Scene1B::UpdateScene()
{
	// {メッシュの更新}
	for (int i = 0; pMesh[i] != NULL; i++)
	{
	...
	// {レイ交差更新}
	if (pCameraRay->RayCollision(pTPCamera->GetTargetPosition(), pTPCamera->GetCameraDirection(true), SetXfile(4)->GetMesh(), pMesh[0]))
	{
	...
	if (pCharacterRayZ->RayCollision(pSkinMesh[0]->vPosition, pSkinMesh[0]->vAxisZ, SetXfile(4)->GetMesh(), pMesh[0]))
	{
	...
	if (pCharacterRayX->RayCollision(pSkinMesh[0]->vPosition, pSkinMesh[0]->vAxisX, SetXfile(4)->GetMesh(), pMesh[0]))
	{
...
//-----------------------------------------------------------------------------
// NAME: VOID Scene1B::RenderStage()
// DESC: ステージシーン処理 ステージシーンの総描画
//-----------------------------------------------------------------------------
VOID Scene1B::RenderScene()
{
	// カリングをしない
	pDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);

	// {メッシュの描画}
	pMesh[0]->RenderMesh(SetXfile(4));
	// {スキンメッシュの描画}
...
//-----------------------------------------------------------------------------
// NAME: VOID Scene1B::DebugImgui()
// DESC: ステージ1処理 ステージデバッグ用IMGUI
//-----------------------------------------------------------------------------
VOID Scene1B::DebugImgui()
{
